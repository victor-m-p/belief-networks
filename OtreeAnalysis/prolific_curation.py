...
df = pd.read_csv('prolific/all_apps_wide-2025-06-23.csv')
...
# for some reason remove first row
df = df.iloc[1:]
n_rows = len(df)

# --- DEMOGRAPHICS ---
code = [df[f'participant.code'].iloc[i] for i in range(n_rows)]
age = [df[f'{base_str}.age'].iloc[i] for i in range(n_rows)] # never got there
gender = [df[f'{base_str}.gender'].iloc[i] for i in range(n_rows)] # never got there
edu = [df[f'{base_str}.education'].iloc[i] for i in range(n_rows)] # never got there
pol = [df[f'{base_str}.politics'].iloc[i] for i in range(n_rows)] # never got there
state = [df[f'{base_str}.state'].iloc[i] for i in range(n_rows)] # never got there
zipcode = [df[f'{base_str}.zipcode'].iloc[i] for i in range(n_rows)]

# --- ANSWERS TO QUESTIONS --- (all looks good)
answer1 = [df[f"{base_str}.answer1"].iloc[i] for i in range(n_rows)] # looks good
answer2 = [df[f"{base_str}.answer2"].iloc[i] for i in range(n_rows)] # looks good
answer3 = [df[f"{base_str}.answer3"].iloc[i] for i in range(n_rows)] # looks good
answer4 = [df[f"{base_str}.answer4"].iloc[i] for i in range(n_rows)] # looks good

# --- MEAT SCALE --- (all looks good)
meat_present = [df[f'{base_str}.meat_consumption_present'].iloc[i] for i in range(n_rows)] # looks good (also spread) 
meat_past = [df[f'{base_str}.meat_consumption_past'].iloc[i] for i in range(n_rows)] # looks good
meat_future = [df[f'{base_str}.meat_consumption_future'].iloc[i] for i in range(n_rows)] # looks good

# --- SOCIAL CIRCLE ---
meat_social = [json.loads(df[f'{base_str}.social_circle_distribution'].iloc[i]) for i in range(n_rows)]

# --- LLM ---
prompt_used = [df[f'{base_str}.prompt_used'].iloc[i] for i in range(n_rows)] # save this ...
llm_result = [json.loads(df[f'{base_str}.llm_result'].iloc[i]) for i in range(n_rows)] # all empty somehow 
generated_nodes = [json.loads(df[f'{base_str}.generated_nodes'].iloc[i]) for i in range(n_rows)] # all empty somehow 
final_nodes = [json.loads(df[f'{base_str}.final_nodes'].iloc[i]) for i in range(n_rows)]
